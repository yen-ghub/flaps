...
st.title("Delay Rate Prediction: Nowcasting")
st.markdown(
    "Nowcasting uses available data up to (and including) the prediction month itself to predict the delay rate."
    "Select a route, airline, and month in the sidebar to compare predictions "
    "against the actual reported value."
)
...
route_airlines = sorted(
    df[df['route'] == selected_route]['airline'].unique().tolist()
)
...
if 'delay_rate_lag2' in row.index and pd.notna(row.get('delay_rate_lag2')):
    ...
    idx = ar_sorted.index.get_loc(row.name)
    lag2_value = ar_sorted.iloc[idx - 1]['delay_rate'] if idx >= 1 else lag1_value
...
# --- Display results ---
st.subheader("Regression: Predicted vs Actual Delay Rate")

# Determine number of columns based on available models
reg_models = [("Ridge", ridge_pred), ("Random Forest", rf_pred)]
# Check if it is the version that has neural network
if has_nn:
...
# Compute average probability across all classifiers for the gauge
all_proba = [logreg_proba, rf_clf_proba]
if has_xgb:
    all_proba.append(xgb_proba)
if has_nn:
    all_proba.append(nn_clf_proba)
...
- **Previous month delay rate (lag1):** {lag1_value:.1%} — most influential feature (68% importance)
- **Same month last year (lag12):** {lag12_value:.1%} — 2nd most influential feature (9.5% importance)
- **Delay rate gradient:** {gradient:+.1%} — {'improving' if gradient < 0 else 'worsening' if gradient > 0 else 'stable'} trend
- **Scheduled sectors:** {int(row['sectors_scheduled'])}
- **Rainy days at arrival (exp):** {row['rainy_days_arr_exp']:.2f}
- **Extreme weather days (total):** {row['extreme_weather_days_total']:.1f}
""")
