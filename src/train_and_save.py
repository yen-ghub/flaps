...
from src.data_loader import MODELS_DIR, load_training_data
from src.feature_engineering import build_feature_matrix, split_data
...
def train_and_save():
    ...
    print("\nSaving models...")
    joblib.dump(ridge, os.path.join(MODELS_DIR, 'ridge_regressor.pkl'))
    joblib.dump(rf_reg, os.path.join(MODELS_DIR, 'rf_regressor.pkl'))
    joblib.dump(logreg, os.path.join(MODELS_DIR, 'logreg_classifier.pkl'))
    joblib.dump(rf_clf, os.path.join(MODELS_DIR, 'rf_classifier.pkl'))
    if xgb_clf is not None:
        joblib.dump(xgb_clf, os.path.join(MODELS_DIR, 'xgb_classifier.pkl'))
    joblib.dump(scaler, os.path.join(MODELS_DIR, 'scaler.pkl'))
    if nn_reg is not None:
        nn_reg.save(os.path.join(MODELS_DIR, 'nn_regressor.keras'))
    if nn_clf is not None:
        nn_clf.save(os.path.join(MODELS_DIR, 'nn_classifier.keras'))

    # Save test set predictions for app visualisations
    test_predictions = {
