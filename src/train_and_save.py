...
from src.data_loader import FORECASTING_MODELS_DIR, NOWCASTING_MODELS_DIR, load_training_data
...
def train_and_save_forecasting():
    """Train forecasting models (only prior months' data) and save to models/forecasting/."""
    ...
    result = build_forecasting_feature_matrix(df)
    df_processed = result['df']
    feature_names = result['feature_names']
    airline_cols = result['airline_cols']
    route_cols = result['route_cols']
